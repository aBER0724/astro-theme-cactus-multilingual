...
// Resolve the canonical site URL in priority order:
//   1. PUBLIC_SITE_URL                — manual override (preview/staging, custom domain anywhere)
//   2. VERCEL_PROJECT_PRODUCTION_URL   — Vercel production custom domain (auto)
//   3. VERCEL_URL                      — current Vercel deployment URL (auto)
//   4. CF_PAGES_URL                    — Cloudflare Pages deployment URL (auto, cf.pages.dev)
//   5. siteConfig.url                  — local default
...
const siteUrl =
	process.env.PUBLIC_SITE_URL
		? withTrailingSlash(process.env.PUBLIC_SITE_URL)
		: process.env.VERCEL_PROJECT_PRODUCTION_URL
			? `https://${process.env.VERCEL_PROJECT_PRODUCTION_URL}/`
			: process.env.VERCEL_URL
				? `https://${process.env.VERCEL_URL}/`
				: process.env.CF_PAGES_URL
					? withTrailingSlash(process.env.CF_PAGES_URL)
					: siteConfig.url;
